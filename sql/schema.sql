...
-- 9) 캘린더 일정 (기념일/여행/일정/기타, 연박 지원)
create table if not exists events (
  id          bigint generated by default as identity primary key,
  category    text not null default 'schedule', -- anniversary | travel | schedule | etc
  title       text not null,
  start_date  date not null,
  end_date    date,                             -- 비우면 당일, 있으면 연박
  emoji       text default '📌',
  owner       text,                             -- 이메일 | 'both'
  repeat      text default 'none',              -- 기념일 반복용
  note        text default '',
  created_by  text,
  created_at  timestamptz default now()
);
...
begin
  foreach t in array array[
    ...
    'pokes','photos','footprints','roulette_options','events'
  ]
  loop
    begin
...
-- 기존 기념일(anniversaries)을 events 로 1회 이전 (재실행 시 중복 방지)
insert into events (category, title, start_date, emoji, repeat, created_at)
select 'anniversary', title, date, coalesce(emoji,'💗'), coalesce(repeat,'none'), created_at
from anniversaries
where not exists (select 1 from events where category = 'anniversary');
